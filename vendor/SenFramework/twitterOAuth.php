...
class twitterOAuth {
	...
	public
	function reconfigure( $config = array() ) {
	...
	private
	function reset_request_settings( $options = array() ) {
	 ...
	 */
	private
	function nonce( $length = 12, $include_time = true ) {
	 ...
	 */
	private
	function prepare_oauth1_params() {
		$defaults = array(
	 ...
	 */
	private
	function multipart_escape( $value ) {
	 ...
	 */
	private
	function prepare_oauth_signature() {
	 ...
	 */
	public
	function bearer_token_credentials() {
	 ...
	 */
	public
	function request( $method, $url, $params = array(), $useauth = true, $multipart = false, $headers = array() ) {
		$options = array(
			'method' => $method,
			'url' => $url,
			'params' => $params,
	...
	public
	function apponly_request( $options = array() ) {
	...
	public
	function user_request( $options = array() ) {
		$options = array_merge( $this->default_options(), $options, array(
			'with_user' => true,
		) );
		$this->reset_request_settings( $options );
	...
	public
	function unauthenticated_request( $options = array() ) {
		$options = array_merge( $this->default_options(), $options, array(
			'with_user' => false,
		) );
		$this->reset_request_settings( $options );
	 ...
	 */
	public
	function url( $request, $extension = 'json' ) {
