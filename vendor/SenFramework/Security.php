...
class Security {
	...
	private function store_csrf($key, $value) {
		if (isset($_SESSION)) {
			$_SESSION[$key]=$value;
		}
	}
	
	public function unset_stored_csrf($key) {
		$_SESSION[$key]=NULL;
		
		unset($_SESSION[$key]);
	}
	
	public function generate_csrf_token($fieldName = NULL) {
		$name = "CSRF_SEN_".((!empty($fieldName)) ? $fieldName.'_' : NULL)."Guard_".mt_rand(0,mt_getrandmax());
		$token = bin2hex(\random_bytes(64));
		
		self::store_csrf($name, $token);
		
		return [$name, $token];
	}
	
	public function validate_csrf_token($form_name, $token_value, $unset = true) {
		$token = self::get_stored_csrf($form_name);
		...
		if($unset === true) {
			self::unset_stored_csrf($form_name);
		}
		
		return $result;
	}
	
	public function get_stored_csrf($key) {
