...
namespace SenFramework\FilePond;

use SenFramework\FilePond\Item;

class RequestHandler {

    // the default location to save tmp files to
    public static $tmp_dir = 'img' . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR;
    ...
    public static function catchExceptions() {
        set_exception_handler('\SenFramework\FilePond\RequestHandler::handleException');
    }
    ...
    private static function createItem($args) {
        return new \SenFramework\FilePond\Item($args);
    }
    ...
    private static function loadBase64FormattedFiles($fieldName) {
        ...
        */
        ...
        if (!$request->is_set_post([$fieldName])) {
            return $items;
        }

        // Handle posted files array
        $values = $request->untrimmed_variable($fieldName, '', true);
    ...
    public static function save($files, $path = 'uploads' . DIRECTORY_SEPARATOR) {
        ...
        // is single item
        return self::saveFile((string)$files, $path);
    }
